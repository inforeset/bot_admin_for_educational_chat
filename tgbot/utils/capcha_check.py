...
async def check_captcha(call: CallbackQuery, config: Config):
    ...
    if await uiic.redis_check_user_id() and await uiic.is_chat_member():
        if  int(call.data.split(':')[1]) == await config.redis_worker.get_capcha_key(uiic.id_user()):
            ...
            logger.info(f"User id:{uiic.id_user()} name:{uiic.obj.message.from_user.full_name()} was pass")
            ...
            logger.info(f"New User id:{uiic.id_user()} name:{uiic.obj.message.from_user.full_name()} was greeting")
